    ...
    public function rules()
    {
        ...
        if (isset($data['skus'])) {
            foreach ($data['skus'] as $index => $value) {
                ...
                // 产品sku
                $key = 'skus.'.$index.'.selected_product_skus';
                foreach ($value['selected_product_skus'] as $product_id => $product_sku_id) {
                    $rules[$key.'.'.$product_id] = 'required';
                    $this->custom_messages[$key.'.'.$product_id.'.required'] = trans('goods::goods.please_select_product_sku');
                }
